// The Legend of Zelda - Iconic Nintendo adventure franchise
export const zeldaData = {
  gameInfo: {
    name: "The Legend of Zelda",
    tagline: "Nintendo's iconic adventure series",
    developer: "Nintendo",
    releaseYear: 1986,
    totalMinerals: 9,
    totalSpecimens: 4,
    diversityScore: "Low",
    gameDescription: "In The Legend of Zelda series, particularly Breath of the Wild and Tears of the Kingdom, players gather 9 different minerals including Amber, Opal, Topaz, Ruby, Sapphire, Diamond, and various ores. These materials are used for upgrading armor, selling to merchants, and crafting jewelry that provides special effects and resistances."
  },
  minerals: [
    {
      id: "opal-1",
      name: "Opal",
      formula: "SiO₂·nH₂O",
      specimenId: "20561",
      type: "rock",
      crystalSystem: "Amorphous (hydrated silica)",
      hardness: "5.5-6.5",
      color: "White, colorless, various with play-of-color",
      description: "Hydrated silica forming colorful internal diffraction patterns. Precious opal displays 'play-of-color' from light diffraction through silica spheres. In Zelda games, opals are valuable gems used for selling or upgrading.",
      inGameUse: "Valuable gemstone for selling to merchants, armor upgrades (Breath of the Wild/Tears of the Kingdom)",
      realWorldUse: "Gemstone jewelry, particularly from Australia (95% of world supply)",
      rarity: "Uncommon",
      locality: "N/A",
      imageUrl: "/TGMS2026/images/zelda/opal-1.jpg",
      funFact: "Opals can contain up to 20% water! In Breath of the Wild, opals are worth 60 rupees each."
    },
    {
      id: "opal-2",
      name: "Opal (Fire Opal)",
      formula: "SiO₂·nH₂O",
      specimenId: "33174",
      type: "rock",
      crystalSystem: "Amorphous",
      hardness: "5.5-6.5",
      color: "Orange, red, yellow",
      description: "Fire opal is a transparent to translucent opal with warm body colors ranging from yellow through orange to red. May or may not show play-of-color. Mexico is the primary source.",
      inGameUse: "Gemstone for trading and armor enhancement",
      realWorldUse: "Collector gemstone, jewelry (especially popular in Mexico)",
      rarity: "Uncommon",
      locality: "N/A",
      imageUrl: "/TGMS2026/images/zelda/opal-2.jpg",
      funFact: "Ancient Romans believed opals brought good luck, while some cultures thought they could make you invisible!"
    },
    {
      id: "amethyst-gem",
      name: "Amethyst",
      formula: "SiO₂",
      specimenId: "33446",
      type: "gem",
      crystalSystem: "Trigonal (Hexagonal)",
      hardness: "7",
      color: "Purple, violet",
      description: "Purple variety of quartz, colored by iron and natural irradiation. The most valuable quartz variety. In Zelda, amethyst is often a mid-tier valuable gemstone.",
      inGameUse: "Valuable gem for selling (Skyward Sword, Breath of the Wild), armor upgrades",
      realWorldUse: "Gemstone jewelry, February birthstone, meditation and decorative stones",
      rarity: "Common",
      locality: "N/A",
      imageUrl: "/TGMS2026/images/zelda/amethyst-gem.jpg",
      funFact: "The name 'amethyst' comes from Greek 'amethystos' meaning 'not intoxicated'—ancient Greeks believed it prevented drunkenness!"
    },
    {
      id: "amethyst-mineral",
      name: "Amethyst Cluster",
      formula: "SiO₂",
      specimenId: "55690",
      type: "mineral",
      crystalSystem: "Trigonal (Hexagonal)",
      hardness: "7",
      color: "Deep purple crystal points",
      description: "Natural amethyst forming crystal clusters (geodes). These spectacular formations occur when quartz crystallizes in gas cavities within volcanic rocks. The deepest purple is most valuable.",
      inGameUse: "Mining resource, decorative element",
      realWorldUse: "Museum specimens, decorative geodes, large specimens for crystal collections",
      rarity: "Common",
      locality: "N/A",
      imageUrl: "/TGMS2026/images/zelda/amethyst-mineral.jpg",
      funFact: "The largest amethyst geode ever found weighs over 13,000 pounds and is displayed in Australia. If you heat amethyst, it turns yellow (citrine)!"
    }
  ]
};
